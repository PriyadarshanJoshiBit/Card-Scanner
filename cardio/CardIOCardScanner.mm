...
- (id)init {
  if((self = [super init])) {
    ...
    self.os = [[OCREScanner alloc] initScanner];
...
- (void)addFrame:(CardIOIplImage *)y
 ...
 brightnessScore:(float)brightnessScore
  ...
  {
    rejectionCount++;
    if( result.focus_score >= 9.0f && rejectionCount >= 10)
    {
      rejectionCount = 0;
      UIImage * image = [y UIImage];
      [os recognizeImageWithTesseract: image];
      if([os hasResult])
      {
        NSMutableString * cardInfoResult = [os getResult];
